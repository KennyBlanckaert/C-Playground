...
class Graph {
    public:
        ...
        /* false = not connected
         * true = strongly connected
         * 
         * Start DFS on a random node
         * If all nodes are visited (twice for directed), the graph is strongly connected
         */
        bool isConnected() {
            if (direction == Direction::UNDIRECTED) {
                vector<int> depthFirstSearchOrder = depthFirstSearch();
                if (find(depthFirstSearchOrder.begin(), depthFirstSearchOrder.end(), -1) != depthFirstSearchOrder.end()) {
                    return false;
                }
                ...
                for (int i = 0; i < countNodes(); i++) { 
                    if (depthFirstSearchOrder[i] > depthFirstSearchOrder[maxPosition]) { maxPosition = i; }
                }

                depthFirstSearchOrder = depthFirstSearch(maxPosition);
                if (find(depthFirstSearchOrder.begin(), depthFirstSearchOrder.end(), -1) != depthFirstSearchOrder.end()) {
                    return false;
                }
